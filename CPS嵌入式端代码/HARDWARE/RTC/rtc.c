...
//hour,min,sec:小时,分钟,秒钟
void RTC_Set_Alarm(u16 RTC_Alarm,u8 week,u8 hour,u8 min,u8 sec)
{ 
	EXTI_InitTypeDef   EXTI_InitStructure;
	RTC_AlarmTypeDef RTC_AlarmTypeInitStructure;
	RTC_TimeTypeDef RTC_TimeTypeInitStructure;
	
	RTC_AlarmCmd(RTC_Alarm,DISABLE);//关闭闹钟A 
  ...
  RTC_SetAlarm(RTC_Format_BIN,RTC_Alarm,&RTC_AlarmTypeInitStructure);
  ...
  EXTI_ClearITPendingBit(EXTI_Line17);//清除LINE17上的中断标志位 
	...
	RTC_AlarmCmd(RTC_Alarm,ENABLE);//开启闹钟A 
...
{ 
	...
	RTC_SetWakeUpCounter(cnt - 1);//设置WAKE UP自动重装载寄存器
