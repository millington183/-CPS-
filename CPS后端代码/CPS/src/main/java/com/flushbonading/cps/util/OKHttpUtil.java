...
import com.flushbonading.cps.entity.DataReleased;
...
//    }
    ...
    public RespBean postData(String... commands) {
        //初始化url
        String url = initUrl();
    ...
    private RespBean post(String url, String json) {
        //发送并得到响应结果
        String responseBody = "";
        ...
        try (Response response = this.client.newCall(request).execute()) {
            if (response.isSuccessful()) {
                return RespBean.success(Tip.POST_SUCCESS.content(), response.body().string());
            }
        } catch (Exception e) {
            log.error("{}", e);
        }
        return RespBean.success(Tip.POST_SUCCESS.content(), responseBody);
    }
    ...
    private String initJSON(String[] commands) {
        ...
        List<DataReleased> datas = new ArrayList<>();
        System.out.println();
        for (String command : commands) {
            datas.add(DataReleased.builder()
